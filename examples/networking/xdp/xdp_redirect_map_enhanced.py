...
flags = 0
def usage():
    print("Usage: {0} <in ifdev> <out ifdev> [<iter count>]".format(sys.argv[0]))
    print("e.g.: {0} eth0 eth1 2\n".format(sys.argv[0]))
    exit(1)
...
in_if = sys.argv[1]
out_if = sys.argv[2]
iter_count = int(sys.argv[3]) if len(sys.argv) == 4 else 0
...
int xdp_redirect_map(struct xdp_md *ctx) {
    ...
    for (int i = 0; i < ITER_COUNT; i++) {
        long *dummy_val = dummy.lookup(&key);
        if (!dummy_val) {
            return XDP_DROP;
        }
        *dummy_val++;
...
""", cflags=["-w", "-DITER_COUNT=" + str(iter_count),
             "-DSAME_IF=" + ("1" if out_if == in_if else "0")])
